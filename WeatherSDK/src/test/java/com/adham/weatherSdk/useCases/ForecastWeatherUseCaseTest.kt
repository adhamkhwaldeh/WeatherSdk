...
class ForecastWeatherUseCaseTest {
    ...
    fun `Successful weather forecast retrieval`() =
        runTest {
            ...
            coEvery { weatherRepository.forecast("London", 24, apiKey) } returns
                Result.success(
                    expectedResponse,
                )
...
//    @Test
//    fun `Extreme hours parameter values`() =
//        runTest {
...
//        }
    ...
    @Test
    fun `Null data transformation via asBasState`() =
        runTest {
            val params = ForecastWeatherUseCaseParams("London", 24)
            every { weatherLocalRepository.getApiKey() } returns "key"
    ...
    @Test
    fun `Flow emission completeness`() =
        runTest {
            every { weatherLocalRepository.getApiKey() } returns "key"
    ...
    fun `Concurrent execution safety`() =
        runTest {
            val emptyResponse = mockk<ForecastResponse>()
            every { weatherLocalRepository.getApiKey() } returns "key"
            coEvery { weatherRepository.forecast(any(), any(), any()) } returns
                Result.success(
                    emptyResponse,
                )
...
//    @Test
//    fun `Repository timeout handling`() =
//        runTest {
//        every { weatherLocalRepository.getApiKey() } returns "key"
